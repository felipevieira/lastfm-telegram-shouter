      ...
      if (c_track):
        ...
        if (curUser[1].get('scrobbles') != user_scrobbles):
          if config.VIEW_MODE == "spotify" and info_from_spotify:
            m = bot.sendMessage(g_id, "User @%s is listening to <a href='%s'>%s</a> by <a href='%s'>%s</a> according to their Last.FM account <a href='%s'>%s</a> " % (userHandle, c_url, c_title, c_artist_url, c_artist, userURL, curUser[0]), parse_mode='HTML')
          else:
          ...
          if config.VIEW_MODE == "spotify" and info_from_spotify:
            m = bot.editMessageText((g_id, m_id), "User @%s is listening to <a href='%s'>%s</a> by <a href='%s'>%s</a> according to their Last.FM account <a href='%s'>%s</a> " % (userHandle, c_url, c_title, c_artist_url, c_artist, userURL, curUser[0]), parse_mode='HTML')
          else:
